  ...
  _onPress: function() {
    if (this.props.onPress) {
      ...
      if (Configuration.getIsReportCrash()) {
        ...
        //console.log(this._reactInternalInstance._currentElement._owner._currentElement);
          actionValue = ''
    ...
    if (!flag) {
      ...
      if (superClassName != 'StaticRenderer') {
        ...
        sqlMngrObj.addUserStep(
        {
          ...
          methodName: actionValue.indexOf('\n') > -1 ? 'Filter item' : actionValue,
          actionOn: actionOn
        });
      } else if (actionOn.indexOf(':') > -1)
        {
          tracker.trackEvent(superClassName, (JSON.stringify(actionOn) == '') ? JSON.stringify(actionValue) : JSON.stringify(actionOn));
          sqlMngrObj.addUserStep(
          {
